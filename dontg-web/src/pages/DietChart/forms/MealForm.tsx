...
const MealForm: React.FC<{ onAddSuccessful?: () => void; onClose?: () => void; isEditing?: boolean; editingItem?: DetailsMeals }> = ({ onAddSuccessful, onClose, isEditing, editingItem }) => {
  ...
  const formSubmit = async (values: any) => {
    ...
    if (!user) return;
    if (!chartId) return;
    if (isEditing && editingItem) await updateDoc(doc(db, "users", user.id, "charts", chartId, "meals", editingItem.id), { name: values.name });
